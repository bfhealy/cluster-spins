...
def find_neighbors(cluster_underscore, proba=0.68, separation=42, mag_cutoff=17, existing=False):
    '''
    Given a target cluster, membership probability threshold and max. angular sepatation (arcsec), use Gaia to find nearby stars that may be blended in TESS.
    '''

    # Read in Cantat-Gaudin et al. 2018 cluster membership file
    cg_mems = ascii.read('/Users/bhealy/Documents/PhD_Thesis/' + cluster_underscore + '/' + cluster_underscore + '_allCGmems.dat')

    # Select stars greater than desired membership probability threshold, must have existing BP-RP color
    cg_mems = cg_mems[(cg_mems['proba'] > proba) & (~np.isnan(cg_mems['bp_rp']))]

    # Assign variables containing source id, G magnitude, ra, dec
    gaia_ids = cg_mems['source_id']
    gmags = cg_mems['phot_g_mean_mag']
    ra = cg_mems['ra']*u.deg
    dec = cg_mems['dec']*u.deg

    # Create astropy SkyCoord collection of each ra, dec
    SC = SkyCoord(ra,dec)
    count = np.zeros(len(SC))

    # Initialize lists for close star ids, magnitudes and angular separations
    closeids = []
    closemags = []
    closeseps = []

    # If there is no existing list of neighbor stars, find them
    if not existing:
        for i in range(len(SC)):
            print(i)
            # Perform a Gaia query of each neighbor star, allowing for timeouts
            for attempt in range(10):
                try:
                    qry = Gaia.query_object_async(SC[i],radius=separation*u.arcsec)
                    break
                except:
                    print('Continuing.')
                    continue

            # Convert distance to TESS pixel size
            qry['dist'] = qry['dist'] / (21./3600)

            # Only include stars brighter than the cutoff magnitude
            nearby_bright = qry[qry['phot_g_mean_mag'] <= mag_cutoff][1:]
            count[i] = len(nearby_bright)

            # Add close stars (if any) to the lists of source id, magnitude and separation
            if count[i] != 0:
                closeids += [nearby_bright['source_id'].data.tolist()]

                # Round off magnitudes to 1 decimal place
                cm1=[]
                for j in range(len(nearby_bright)):
                    cm1 += [np.float(format(nearby_bright['phot_g_mean_mag'].data[j],'.1f'))]
                closemags += [cm1]

                # Round off separations to 1 decimal place
                closeseps += [np.round(nearby_bright['dist'].data,1).tolist()]

            # If no neighboring stars bright enough, fill lists with -1 placeholder
            else:
                closeids += [[-1]]
                closemags += [[-1]]
                closeseps += [[-1]]

            # Write results as a pandas dataframe
            df = pd.DataFrame(data={'closeids':closeids[i],'closemags':closemags[i],'closeseps':closeseps[i]})
            df.to_csv('/Users/bhealy/Documents/PhD_Thesis/' + cluster_underscore + '/' + 'close_stars/' + np.str(gaia_ids[i])+'.csv',index=False)

    # If there is already an existing collection of neighbors, read it in
    else:
        for i in range(len(SC)):
            print(i)

            df = pd.read_csv('/Users/bhealy/Documents/PhD_Thesis/' + cluster_underscore + '/' + 'close_stars/' + np.str(gaia_ids[i])+'.csv')

            closeids += [df['closeids'].values.tolist()]
            closemags += [df['closemags'].values.tolist()]
            closeseps += [df['closeseps'].values.tolist()]

    return closeids, closemags, closeseps
...
def get_lc_and_period(n,mems,size=(31,31),sigma_thresh=5,tpfplot=False,sec=0):
    ...
    lcdirs = glob.glob('/Users/bhealy/Documents/PhD_Thesis/NGC_2516/CDIPS/mastDownload/HLSP/*'+np.str(gaia_ids[n])+'*')
    ...
    if len(lcdirs) != 0: #PROCEED WITH CDIPS
        print('CDIPS')
        for d in lcdirs:
    ...
    #print(all_maxheights, secs)
        ...
        tempdata = eleanor.TargetData(S, height=size[0], width=size[1], bkg_size=31, do_psf=False, do_pca=False, crowded_field=True)
...
label = np.zeros(len(ngc2516mems),dtype=int)

#perTbl = Table(data=[gaia_ids,periods,period_uncs,e_period_hwhm,e_period_mad,e_period_std,label],names=['source_id','period','period_unc','e_period_hwhm','e_period_mad','e_period_std','CDIPS'])
#perTbl = ascii.read('/Users/bhealy/Documents/PhD_Thesis/NGC_2516/NGC_2516_ptbl_newf.dat')
perTbl = ascii.read('/Users/bhealy/Documents/PhD_Thesis/NGC_2516/cdips_lcgen/NGC_2516_ptbl_cdips_neighbors.dat')
...
#for n in range(955,len(ngc2516mems)):
...
#ssl.SSLError
    ...
    lc_pca, TPF, apers, periods[n], period_uncs[n], e_period_hwhm[n], e_period_mad[n], e_period_std[n], tau, emp_acorr_smooth, peakinds, valinds, maxheight, t_start, t_end, periodmults, pos, label[n], bestsec = get_lc_and_period(n,ngc2516mems)
    ...
    lctbl.write('/Users/bhealy/Documents/PhD_Thesis/NGC_2516/cdips_lcgen/Figs/Light_Curves/'+np.str(n)+'_'+gaia_id+'_lc.txt',format='ascii',overwrite=True)

    pdgtbl = pdg.to_table()
    pdgtbl.write('/Users/bhealy/Documents/PhD_Thesis/NGC_2516/cdips_lcgen/Figs/Periodograms/'+np.str(n)+'_'+gaia_id+'_pdg.txt',format='ascii',overwrite=True)
    ...
    if -1 not in closeids[n]:
       ...
       for m in range(len(closeids[n])):
           if closeids[n][m] in ngc2516mems['source_id']:
               clr = 'green'
    ...
    fig2.savefig('/Users/bhealy/Documents/PhD_Thesis/NGC_2516/cdips_lcgen/CDIPS_LC_DVRs_neighbors/'+np.str(n)+'_'+gaia_id+'_dvr.pdf',overwrite=True,bbox_inches='tight')
    plt.close('all')
    ...
    if -1 not in closeids[n]:
       ...
       for m in range(len(closeids[n])):
           ...
           #print(n,m)
           lcdirs = glob.glob('/Users/bhealy/Documents/PhD_Thesis/NGC_2516/CDIPS/mastDownload/HLSP/*'+np.str(closeids[n][m])+'*')
           ...
           lctbl.write('/Users/bhealy/Documents/PhD_Thesis/NGC_2516/cdips_lcgen/Figs/Neighbor_Light_Curves/'+np.str(n)+'_'+gaia_id+'_lc.txt',format='ascii',overwrite=True)

           pdgtbl = pdg.to_table()
           pdgtbl.write('/Users/bhealy/Documents/PhD_Thesis/NGC_2516/cdips_lcgen/Figs/Neighbor_Periodograms/'+np.str(n)+'_'+gaia_id+'_pdg.txt',format='ascii',overwrite=True)
    ...
    fig3.savefig('/Users/bhealy/Documents/PhD_Thesis/NGC_2516/cdips_lcgen/CDIPS_LC_DVRs_neighbors/'+np.str(n)+'_'+gaia_id+'_neighbors.pdf',overwrite=True,bbox_inches='tight')
    ...
    perTbl['CDIPS'][n] = label[n]

    #perTbl.write('/Users/bhealy/Documents/PhD_Thesis/NGC_2516/NGC_2516_ptbl_newf.dat',format='ascii',overwrite=True)
    perTbl.write('/Users/bhealy/Documents/PhD_Thesis/NGC_2516/cdips_lcgen/NGC_2516_ptbl_cdips_neighbors.dat',format='ascii',overwrite=True)
